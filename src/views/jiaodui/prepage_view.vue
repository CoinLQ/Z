...
export default {
  ...
  methods: {
    ...
    getWorkingData() {
        let that = this;
        let url = '/api/prepagecoltask/obtain/';

        if (this.$route.params.id) {
          url = '/api/prepagecoltask/' + this.$route.params.id + '/';
        }
        that.$Loading.start();
    ...
    submit() {
        let url = '/api/prepagecoltask/' + this.task_id + '/save/';
        let that = this;
        // that.$Notice.error({
        //     title: that.$t('Failed'),
    ...
    done_submit() {
        let url = '/api/prepagecoltask/' + this.task_id + '/done/';
        let that = this;
        this.isBtnLoading = true;
        document.getElementsByClassName("canvas-layout")[0].focus()
        let r = _.forEach(_.cloneDeep(this.solidRects), function (item) {
        ...
        util.ajax.post(url, {rects: r, current_x: this.curRect.x, current_y: this.curRect.y}).then(function (response) {
            ...
            that.$router.push({ path: '/collate_tasks/prepagetask'});
        }).catch(function (error) {
            that.isBtnLoading = false;
            that.$Notice.error({
                title: that.$t('Failed'),
