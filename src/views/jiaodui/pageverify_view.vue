...
export default {
  ...
  methods: {
    ...
    getWorkingData() {
        let that = this;
        let url = '/api/pageverifytask/obtain/';

        if (this.$route.params.id) {
          url = '/api/pageverifytask/' + this.$route.params.id + '/';
        }
        that.$Loading.start();
    ...
    submit() {
        ...
        util.ajax.post(url, {rects: r, current_x: this.curRect.x, current_y: this.curRect.y}).then(function (response) {
            ...
            that.$router.push({ path: '/collate_tasks/pageverifytask'});
        }).catch(function (error) {
            that.isBtnLoading = false;
            that.$Notice.error({
                title: that.$t('Failed'),
    ...
    done_submit() {
        let url = '/api/pageverifytask/' + this.task_id + '/done/';
        let that = this;
        this.isBtnLoading = true;
        document.getElementsByClassName("canvas-layout")[0].focus()
