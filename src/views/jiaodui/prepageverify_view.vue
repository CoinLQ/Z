...
<div class="outter-wrapper">
  ...
  <Row>
        ...
        <Col span="12">
            <Button type="error" :disabled="status == 7 || status == 0" icon="checkmark-round" long @click="submit" :loading="isBtnLoading">
                <span v-if="!isBtnLoading">退回重做</span>
                <span v-else>进行中</span>
            </Button>
        </Col>
...
export default {
  ...
  methods: {
    ...
    getWorkingData() {
        let that = this;
        let url = '/api/prepagecolverifytask/obtain/';

        if (this.$route.params.id) {
          url = '/api/prepagecolverifytask/' + this.$route.params.id + '/';
        }
        that.$Loading.start();
    ...
    submit() {
        let url = '/api/prepagecolverifytask/' + this.task_id + '/redo/';
        let that = this;
        // that.$Notice.error({
        //     title: that.$t('Failed'),
        ...
        util.ajax.post(url, {rects: r, current_x: this.curRect.x, current_y: this.curRect.y}).then(function (response) {
            ...
            that.$Notice.success({title: '随喜', desc: '任务已退回'});
            that.$router.push({ path: '/collate_tasks/prepageverifytask'});
        }).catch(function (error) {
            that.isBtnLoading = false;
            that.$Notice.error({
                title: that.$t('Failed'),
    ...
    done_submit() {
        let url = '/api/prepagecolverifytask/' + this.task_id + '/done/';
        let that = this;
        this.isBtnLoading = true;
        document.getElementsByClassName("canvas-layout")[0].focus()
